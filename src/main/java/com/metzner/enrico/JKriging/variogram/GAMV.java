...
//		double[] csdtol = new double[ndir];
		...
		if(x_var!=null) {x = (double[]) dataframe.getArray(x_var); datalength = Math.max(datalength,x.length); }
		if(y_var!=null) {y = (double[]) dataframe.getArray(y_var); datalength = Math.max(datalength,y.length); }
		if(z_var!=null) {z = (double[]) dataframe.getArray(z_var); datalength = Math.max(datalength,z.length); }
...
//        c MAIN LOOP OVER ALL VARIOGRAMS:
		for(int iv=0; iv<nvarg; iv++) {
			...
			double[] tail = (double[]) dataframe.getArray(ivtail-1+Constants.FIRST_IDX);
			double[] head = (double[]) dataframe.getArray(ivhead-1+Constants.FIRST_IDX);
